...
class RNNGAN(object):
    ...
    def __init__(self, is_training, num_song_features=None, num_meta_features=None):
        ...
        with tf.variable_scope('G') as scope:
            scope.set_regularizer(tf.contrib.layers.l2_regularizer(scale=FLAGS.reg_scale))
            # lstm_cell = tf.contrib.rnn.BasicLSTMCell(FLAGS.hidden_size_g, forget_bias=1.0, state_is_tuple=True, reuse = tf.get_variable_scope().reuse)
            cells = []
            for n in range(FLAGS.num_layers_g):
            ...
            #     lstm_cell = tf.contrib.rnn.DropoutWrapper(
            #         lstm_cell, output_keep_prob=FLAGS.keep_prob)
            # cell = tf.contrib.rnn.MultiRNNCell([lstm_cell for _ in range(FLAGS.num_layers_g)], state_is_tuple=True)
            ...
            for i, input_ in enumerate(random_rnninputs):
                ...
                # function cell is the method under version 1.0
                output, state = cell(input_, state)
                # output, state = tf.nn.dynamic_rnn(cell, input_, initial_state=state)
    ...
    def discriminator(self, inputs, is_training, msg='', ifReuse=None):
        # RNN discriminator:
        ...
        cells_fw = []
        for f in range(FLAGS.num_layers_d):
            lstm_cell = tf.contrib.rnn.BasicLSTMCell(FLAGS.hidden_size_d, forget_bias=1.0, state_is_tuple=True, reuse=ifReuse)
            if is_training and FLAGS.keep_prob < 1:
                lstm_cell = tf.contrib.rnn.DropoutWrapper(
                    lstm_cell, output_keep_prob=FLAGS.keep_prob)
            cells_fw.append(lstm_cell)
        ...
        #     lstm_cell = tf.contrib.rnn.DropoutWrapper(
        #         lstm_cell, output_keep_prob=FLAGS.keep_prob)
        # cell_fw = tf.contrib.rnn.MultiRNNCell([lstm_cell for _ in range(FLAGS.num_layers_d)], state_is_tuple=True)
        self._initial_state_fw = cell_fw.zero_state(self.batch_size, data_type())
        if not FLAGS.unidirectional_d:
            cells_bw = []
            for b in range(FLAGS.num_layers_d):
                lstm_cell = tf.contrib.rnn.BasicLSTMCell(FLAGS.hidden_size_g, forget_bias=1.0, state_is_tuple=True, reuse=ifReuse)
                if is_training and FLAGS.keep_prob < 1:
                    lstm_cell = tf.contrib.rnn.DropoutWrapper(
                        lstm_cell, output_keep_prob=FLAGS.keep_prob)
                cells_bw.append(lstm_cell)
            cell_bw = tf.contrib.rnn.MultiRNNCell(cells_bw, state_is_tuple=True)
            # lstm_cell = tf.contrib.rnn.BasicLSTMCell(FLAGS.hidden_size_g, forget_bias=1.0, state_is_tuple=True)
            # if is_training and FLAGS.keep_prob < 1:
            #     lstm_cell = tf.contrib.rnn.DropoutWrapper(
            #         lstm_cell, output_keep_prob=FLAGS.keep_prob)
            # cell_bw = tf.contrib.rnn.MultiRNNCell([lstm_cell for _ in range(FLAGS.num_layers_d)], state_is_tuple=True)
